...
class gradingform_frubric_controller extends gradingform_controller {
    ...
    public function extend_navigation(global_navigation $navigation, navigation_node $node = null) {
        ...
        if ($this->is_form_defined() && ($options = $this->get_options()) && !empty($options['alwaysshowdefinition'])) {
            $node->add(
                get_string('gradingof', 'gradingform_rubric', get_grading_manager($this->get_areaid())->get_area_title()),
...
class gradingform_frubric_instance extends gradingform_instance {
    ...
    public function validate_grading_element($elementvalue) {
        ...
        foreach ($criteria as $id => $criterion) {
            ...
            $max = 0;
            if (strpos($criterion['levels'][$levelid]['score'], '-') !== false) {
                $max = explode('-', $criterion['levels'][$levelid]['score']);
                $max = $max[count($max) - 1];
            } else if (strpos($criterion['levels'][$levelid]['score'], '/') !== false) {
                $max = explode('/', $criterion['levels'][$levelid]['score']);
                $max = $max[count($max) - 1];
            }
        ...
        if ($currentinstance) {
            ...
            foreach ($curfilling['criteria'] as $criterionid => $curvalues) {
                ...
                if (isset($value['criteria'][$criterionid]['leveljson'])) {
                    ...
                    $this->set_criteria_fillings_level($data, $criterionid, $leveljson);
                }
    ...
    private function set_criteria_fillings_level($criteria, $criteriaid, $leveljson) {
