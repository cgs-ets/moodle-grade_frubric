...
class cron_grade_outcomes extends \core\task\scheduled_task {
    ...
    public function execute() {
        ...
        // Detect frubric saves where at least one criterion has an outcome, using grading_instances.timemodified
        // as the signal. lib.php::update() explicitly touches grading_instances.timemodified when any criterion
        // with outcomeid <> 0 is saved, making this reliable regardless of whether grade_grades was recalculated.
        // startofmonth limits the scan to avoid processing old historical data.
        $this->log("Looking for frubric grading instances modified since last run: $lastrun");
        ...
        $sql = "SELECT DISTINCT
                ...
                INNER JOIN {grading_instances} gri
                    ON gri.itemid = ag.id
                    AND gri.timemodified >= :lastrun
                ...
                AND oc.outcomeid > 0";
        $fgraded = $DB->get_records_sql($sql, ['lastrun' => $lastrun, 'startofmonth' => $startofmonth]);
